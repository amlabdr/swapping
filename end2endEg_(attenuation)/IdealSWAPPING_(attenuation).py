...
def set_parameters(topology: RouterNetTopo,distance, attenuation):
...
def simulate(distance, attenuation):
    
    network_topo = RouterNetTopo(network_config)
    tl = network_topo.get_timeline()
    set_parameters(network_topo,distance,attenuation)
...
attenuationdict = {"0db|km":0,"2db|km":0.002}
attenuationdict = {"1db|km":0.001,"3db|km":0.003}
...
for attenuation in attenuationdict:
    print("simulation fr TCH:", attenuation)
    rates = []
    for L in distances:
        rates.append(simulate(L,attenuationdict[attenuation]))
        print("done for ", L)
    # Save rates to a file
    filename = "end2endEg_(attenuation)/rates/"+attenuation+".txt"
    # Save rates to a file
